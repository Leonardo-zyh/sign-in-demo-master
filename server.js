  ...
  if(path === '/'){
    ...
    readBody(request).then((body)=>{
      ...
      strings.forEach((string)=>{
        ...
        let key = parts[0]
        let value = parts[1]
        hash[key] = decodeURIComponent(value) //翻译字符串中的@,%40  hash['email'] = '956731563@qq.com'
      })
      let {email, password, password_confirmation} = hash
      if(email.indexOf('@') === -1){  //判断语句中没有@，indexOf返回在数组中给定元素索引
        response.statusCode = 400
        response.setHeader('Content-Type', 'application/json;charset=utf-8')//application/json，JSON格式
        response.write(`{  
          "errors": {
            "email": "invalid"
          }
        ...
        try{
          users = JSON.parse(users) //users是个 []
        }catch(exception){
          users = []
        }
        let inUse = false       
